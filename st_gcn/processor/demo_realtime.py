...
class DemoRealtime(IO):
    ...
    def start(self):
        ...
        while(True):
            ...
            if multi_pose is not None:
                ...
                app_fps = 1 / (time.time() - tic)
                image = self.renderEmpty(voting_label_name, intensity, orig_image, app_fps )
    ...
    def renderEmpty(self, voting_label_name, intensity, orig_image, fps=0):
        images = utils.visualization.stgcn_visualize_empty(
            self.model.graph.edge,
            intensity[[-1]],[orig_image],
            voting_label_name,
            None,
