...
class DemoFight(IO):
    ...
    def start(self):
        ...
        self.groundtruth = []
        if(self.arg.mode == "test_video"): self.test_video()
        elif(self.arg.mode == "test_temporal"): self.test_temporal()
        else: self.recognition()

    #Uses a .csv file and the supplied arguments in demo_fight.yaml to calculate F1 Score on a supplied video dataset with temporal-level annotations
    def test_temporal(self):
        with open(self.arg.test_split, newline='') as csvfile:
            csvreader = csv.reader(csvfile, delimiter=' ', quotechar='|')
            #csvreader contains all of the filenames
            for video in csvreader:
                self.predictions = []
                self.videogroundtruth = []
                self.arg.video = self.arg.video_path+video[0]
                annotation = self.arg.annotation_path+video[0]+'.csv'
                
                #Opens the .csv at the path provided by config file, loads in its temporal annotations
                ...
                #Runs demo on this video
                self.recognition()
                self.totalpredictions.extend(self.predictions)

                #To prevent issues if the test fails halfway, results are stored under the prediction folder
                ...
                with open(self.arg.prediction_out+video[0]+'-f1.csv', 'w', newline='') as localf1file:
                    f1 = f1_score(self.videogroundtruth, self.predictions, average='macro')
                    wr = csv.writer(localf1file, quoting=csv.QUOTE_ALL)
                    wr.writerow(f1)
                
        f1 = f1_score(self.groundtruth, self.totalpredictions, average='macro')
        acc = accuracy_score(self.groundtruth, self.totalpredictions)
        self.io.print_log('\tF1 Score: {:.4f}'.format(f1))
        self.io.print_log('\tAccuracy Score {:.4f}'.format(acc))

    #Similar to the temporal level testing above, but only predicts on whole video; if it is violent or not. Takes whichever classification occurs more as the answer.
    def test_video(self):
        with open(self.arg.video_annotation_path, newline='',encoding='utf-8-sig') as csvfile:
            csvreader = csv.reader(csvfile, delimiter=',')
            for videopred in csvreader:
                video = videopred[0]
                print(video)
                pred = videopred[1]
                self.predictions = []
                self.videogroundtruth = []
                self.arg.video = self.arg.video_path+video
                self.groundtruth.append(pred)
                self.recognition()
                prediction = max(set(self.predictions), key = self.predictions.count)

                self.totalpredictions.append(prediction)
                print(str(pred) + ", " + str(prediction))
    ...
    def recognition(self):
        ...
        while(True):
            ...
            # Creates UI elements dependent on state, in the top-left
            ...
            #Processing for when user clicks on UI elements to be interacted with
    ...
    def predict(self, data):
        ...
        # Classification is sum along dimensionality of output
    ...
    # Requires a different render method for when no skeleton detected
    ...
    def get_parser(add_help=False):
        ...
        parser.add_argument('--mode',
                            default='recognition',
                            help='Execution mode of demo; test temporally, test video-level or recognise')
        ...
        parser.add_argument('--video_annotation_path',
                            default='',
                            help='Path to video-level annotations for test set')
