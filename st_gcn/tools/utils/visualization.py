    ...
    for t in range(T):
        ...
        for m in range(M):
            ...
            cv2.putText(text, body_label, pos_track[m],
                        cv2.FONT_HERSHEY_COMPLEX, 0.5 * scale_factor,
                        (255, 255, 255))
...
def stgcn_visualize_notext(video):
    height = 1080
    T = len(video)
    for t in range(T):
        frame = video[t]
...
def stgcn_fight_visualize(showSkeleton,
                    showRGB,
                    pose,
                    edge,
                    feature,
    ...
    for t in range(T):
        ...
        rgb_result = blurred_mask.astype(float) * 0.75
        if(showRGB): rgb_result += frame.astype(float)
        rgb_result += skeleton.astype(float) * 0.25
        ...
        if label is not None:
            label_name = label
            right_align_text_colour(rgb_result, label_name, (0, 0, 255))
            right_align_text_colour(frame, label_name, (0, 0, 255))

        if fps is not None:
            put_text_colour(rgb_result, '{:.2f}fps'.format(fps), (1, 0.125), (0, 0, 255))
            put_text_colour(frame, '{:.2f}fps'.format(fps), (1, 0.125), (0, 0, 255))

        if(showSkeleton): yield rgb_result
        else: yield frame

def stgcn_visualize_fight_empty(showSkeleton,
                    showRGB,
                    video,
                    label=None,
                    height=1080,
    ...
    for t in range(T):
        ...
        if(showRGB): rgb_result = frame.copy()
        else: rgb_result = frame * 0
...
def stgcn_visualize_skeleton(pose,
                    edge,
                    video,
                    height=1080,
                    fps=None):
...
def put_text_colour(img, text, position, colour, scale_factor=1):
    t_w, t_h = cv2.getTextSize(
        text, cv2.FONT_HERSHEY_TRIPLEX, scale_factor, thickness=1)[0]
    H, W, _ = img.shape
    position = (int(W * position[1] - t_w * 0.5),
...
def right_align_text(img, text, scale_factor=1):
    t_w, t_h = cv2.getTextSize(
        text, cv2.FONT_HERSHEY_TRIPLEX, scale_factor, thickness=1)[0]
    H, W, _ = img.shape
    position = (int(W - t_w),
...
def right_align_text_colour(img, text, colour, scale_factor=1):
    t_w, t_h = cv2.getTextSize(
        text, cv2.FONT_HERSHEY_TRIPLEX, scale_factor, thickness=1)[0]
    H, W, _ = img.shape
    position = (int(W - t_w),
