...
namespace op
{
        ...
        mCocoJsonVariant{cocoJsonVariant}
    {
        try
        {
            ...
            {
                std::get<0>(jsonOfstream).objectOpen();
                std::get<0>(jsonOfstream).key("data");
    ...
    CocoJsonSaver::~CocoJsonSaver()
    {
        try
        {
            for (auto& jsonOfstream : mJsonOfstreams)
            {
                ...
                std::get<0>(jsonOfstream).comma();
                
                std::get<0>(jsonOfstream).key("label");
                std::get<0>(jsonOfstream).plainText("\"placeholder1\"");
                std::get<0>(jsonOfstream).comma();

                std::get<0>(jsonOfstream).key("label_index");
                std::get<0>(jsonOfstream).plainText("placeholder2");
                std::get<0>(jsonOfstream).objectClose();
            }
        }
        catch (const std::exception& e)
        {
        ...
        try
        {
            ...
            if (numberPeople > 0)
            {
                ...
                for (auto& jsonOfstreamAndFormat : mJsonOfstreams)
                {
                    ...
                    // build\bin\OpenPoseDemo.exe --write_coco_json output/blah.json keypoint_scale 3 --video examples\media\video.avi
                    // New element
                    jsonOfstream.objectOpen();

                    // image_id
                    jsonOfstream.key("frame_index");
                    jsonOfstream.plainText(imageId+1);
                    jsonOfstream.comma();

                    // keypoints - i.e., poseKeypoints
                    jsonOfstream.key("skeleton");
                    ...
                    for (auto person = 0 ; person < numberPeople ; person++)
                    {
                        ...
                        {
                            if(person!=0) jsonOfstream.comma();
                            jsonOfstream.objectOpen();
                            jsonOfstream.key("pose");
                            jsonOfstream.arrayOpen();
                            //here goes pose keypoints; should be 36 of them (each pair is x,y)
                            for (auto bodyPart = 0u ; bodyPart < indexesInCocoOrder.size() ; bodyPart++)
                            {
                                const auto finalIndex = 3*(person*numberBodyParts + indexesInCocoOrder.at(bodyPart));
                                const auto validPoint = (poseKeypoints[finalIndex+2] > 0.f);
                                jsonOfstream.plainText(validPoint ? poseKeypoints[finalIndex] : 0.f);
                                jsonOfstream.comma();
                                jsonOfstream.plainText(validPoint ? poseKeypoints[finalIndex+1] : 0.f);
                            ...
                            jsonOfstream.arrayOpen();
                            //here goes score points: should be 18 of them (1 score for each keypoint)
                            for (auto bodyPart = 0u ; bodyPart < indexesInCocoOrder.size() ; bodyPart++)
                            {
                                const auto finalIndex = 3*(person*numberBodyParts + indexesInCocoOrder.at(bodyPart));
                                const auto validPoint = (poseKeypoints[finalIndex+2] > 0.f);
                                jsonOfstream.plainText(poseKeypoints[finalIndex+2]);
                                if (bodyPart < indexesInCocoOrder.size() - 1u)
                                    jsonOfstream.comma();
                            }
                            jsonOfstream.arrayClose();
